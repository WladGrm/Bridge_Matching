...
def train_velocity( vnet, to_swiss_roll=True, eps=0.1, epoches = 1000, batch_size = 64, l_r = 1e-3, std=0.5, device = "cpu"):
    """
    Velocity/Drift network training from Gaussian (Normal) to Swiss Roll distribution
    if (to_swiss_roll = True), else - vice versa
    """
    ...
    for i in tqdm(range(epoches)):
        ...
        if to_swiss_roll is True: #From Normal to Swiss Roll
        ...
        else: #From Swiss Roll to Normal
            x_0 = sample_2d_swiss_roll(batch_size=batch_size, std=std).to(device)
            x_1 = sample_2d_gaussian(batch_size=batch_size).to(device)

        #Calculate x_t with added noise N ~ (0, eps*t*(1-t)*I)
        added_noise = torch.sqrt(eps * t * (1. - t)) * torch.randn_like(x_0)
        x_t = t * x_1 + (1. - t) * x_0 + added_noise

        #=======
        #DATA PREP 
        #x_t [BS,[x,y]] concat t [BS,1] --> x_tt [BS,[x,y,t]]
...
class SDE_Solver(nn.Module):
    def __init__(self, steps=100, start_t = 0, end_t = 1, eps=0.1, device="cpu"):
        """
    ...
    def get_velocity(self, x, t, vnet, vnet_backward=None):
        
        with torch.no_grad():

            vnet = vnet.to(self.device)
            x_tt = torch.cat((x, t), dim = 1).to(self.device)

            if vnet_backward is None:
                return vnet(x_tt)
            else:
                vnet_backward = vnet_backward.to(self.device)
            return (vnet(x_tt) - vnet_backward(x_tt)) / 2
    ...
    def solve(self, x, vnet, vnet_backward=None, printable=True):
        """
        ...
        Args:
            ...
            vnet_backward: for experiments (drift from swiss roll to normal)
        ...
        for step in range (self.steps):
                ...
                if vnet_backward is None:
                    ...
                    velocity = self.get_velocity(x, t, vnet, vnet_backward)
